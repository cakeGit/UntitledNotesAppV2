...
import { logDb, logDbIndex } from '../logger.mjs';

logDbIndex("Database module loaded, initializing database...");
...
export const db = new sqlite.Database(path.join(DB_DIR, 'database.sqlite'), async (err) => {
    if (err) {
        ...
        logDbIndex('Connected to database at', path.join(DB_DIR, 'database.sqlite'));
        ...
        db.exec(setupSQL, (err) => {
            if (err) {
                ...
                logDbIndex('Database setup complete, delegating database work to databaseWorker.js');

                requireDatabaseWorker().then((mod) => {
                    mod.startDatabaseWorker(db);
                    logDbIndex('Database worker started');
                }).catch(err => {
